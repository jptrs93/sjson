...
`sjson` is a simple, light weight, single pass parser for working data the is JSON format but of unknown structure. Once parsed, the structure of the JSON is captured in a general purpose struct allowing it to be repeatedly queried and manipulated efficiently. This contrasts with other packages aimed at working with unstructured JSON such as [jsonparser](https://github.com/buger/jsonparser) where the bytes must be parsed upon every query. The parsing is performed in a single pass of the bytes and may be done re-using a byte slice without copying.
